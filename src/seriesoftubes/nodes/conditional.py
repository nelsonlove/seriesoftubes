...
from seriesoftubes.models import ConditionalNodeConfig, Node
from seriesoftubes.nodes.base import NodeContext, NodeExecutor, NodeResult
from seriesoftubes.schemas import ConditionalNodeInput, ConditionalNodeOutput


class ConditionalNodeExecutor(NodeExecutor):
    """Executor for conditional nodes that implement branching logic"""

    input_schema_class = ConditionalNodeInput
    output_schema_class = ConditionalNodeOutput
    ...
    async def execute(self, node: Node, context: NodeContext) -> NodeResult:
        ...
        try:
            ...
            for condition in config.conditions:
                ...
                try:
                    ...
                    return NodeResult(
                        output=None,
                        success=False,
                        error=f"Condition evaluation failed '{condition.condition}': {e!s}",
                    )
            ...
            return NodeResult(
                output=None,
                success=False,
                error=f"Conditional node execution failed: {e!s}",
            )
