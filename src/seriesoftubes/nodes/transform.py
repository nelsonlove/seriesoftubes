...
class TransformNodeExecutor(NodeExecutor):
    ...
    async def execute(self, node: Node, context: NodeContext) -> NodeResult:
        ...
        try:
            ...
            # Get the array data to transform
            if config.field:
            ...
            if isinstance(config.template, dict):
                    ...
                    try:
                        ...
                        for field_name, field_template in config.template.items():
                            if isinstance(field_template, str):
                                ...
                                try:
                                    if rendered_value.strip().startswith(('{', '[', '"')) or rendered_value.strip() in ('true', 'false', 'null'):
                        ...
                        return NodeResult(
                            output=None,
                            success=False,
                            error=f"Template error for item {item}: {str(e)}",
                        )
                ...
                try:
                    ...
                    return NodeResult(
                        output=None,
                        success=False,
                        error=f"Invalid template syntax: {str(e)}",
                    )
                ...
                for item in array_data:
                    ...
                    try:
                        ...
                        try:
                            ...
                            transformed_items.append(parsed_value)
                        except json.JSONDecodeError:
                            transformed_items.append(rendered_value)
            ...
            return NodeResult(
                output=None,
                success=False,
                error=f"Transform node execution failed: {str(e)}",
            )