...
class TestDocsAPI:
    ...
    def test_get_doc_content_outside_docs_dir(self):
        ...
        with patch('pathlib.Path.exists', return_value=True), \
            ...
            mock_resolve.return_value.relative_to.side_effect = ValueError("Path not relative")
    ...
    def test_doc_file_title_extraction(self):
        ...
        with patch('seriesoftubes.api.docs_routes.open', mock_open(read_data=mock_content)):
    ...
    def test_doc_file_category_determination(self):
        ...
        for path, expected_category in test_cases:
            with patch('seriesoftubes.api.docs_routes.open', mock_open(read_data="# Test")):
                doc = DocFile(Path(path), path)
                assert doc.category == expected_category, f"Path {path} should be category {expected_category}"

    def test_doc_file_to_dict(self):
        """Test DocFile serialization to dictionary."""
        from seriesoftubes.api.docs_routes import DocFile
        
        with patch('seriesoftubes.api.docs_routes.open', mock_open(read_data="# Test Title\n\nContent.")):
