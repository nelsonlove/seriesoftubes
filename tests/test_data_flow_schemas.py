...
from seriesoftubes.schemas import (
    SplitNodeInput, SplitNodeOutput,
    AggregateNodeInput, AggregateNodeOutput,
    FilterNodeInput, FilterNodeOutput,
    TransformNodeInput, TransformNodeOutput,
    JoinNodeInput, JoinNodeOutput,
    ForeachNodeInput, ForeachNodeOutput,
    ConditionalNodeInput, ConditionalNodeOutput,
    NODE_SCHEMAS
)
...
class TestDataFlowSchemas:
    ...
    def test_aggregate_node_schemas(self):
        ...
        # Valid output
        valid_output = AggregateNodeOutput(
            result=[{"a": 1}, {"b": 2}],
            count=2
        )
    ...
    def test_filter_node_schemas(self):
        ...
        # Valid input
        valid_input = FilterNodeInput(
            items=[1, 2, 3],
            filter_context={"threshold": 2}
        )
        assert valid_input.items == [1, 2, 3]
        assert valid_input.filter_context["threshold"] == 2
        
        # Valid output
        valid_output = FilterNodeOutput(
            filtered=[2, 3],
            removed_count=1
        )
    ...
    def test_foreach_node_schemas(self):
        ...
        valid_input = ForeachNodeInput(
            items=[1, 2, 3],
            foreach_context={"multiplier": 2}
        )
        assert len(valid_input.items) == 3
        
        # Valid output
        valid_output = ForeachNodeOutput(
            results=[2, 4, 6],
            execution_count=3
        )
    ...
    def test_all_nodes_have_schemas(self):
        ...
        expected_nodes = [
            "split", "aggregate", "filter", "transform", 
            "join", "foreach", "conditional",
            "llm", "http", "file", "python", "route"  # Including existing nodes
        ]
        
        for node_type in expected_nodes:
            assert node_type in NODE_SCHEMAS, f"Node type '{node_type}' missing from NODE_SCHEMAS"
